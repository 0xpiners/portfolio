...
They give us the email address *ctf-player@picoctf.org*, and also a list of passwords.
![[Pasted image 20251028153843.png]]
We can try to *brute-force the password*, but the description tells us there's a *rate-limiting restriction*. However, we can still try using *user-controlled headers*.

For that I wrote this script:
![[Pasted image 20251028222204.png]]

What this script does is use the header *X-Forwarded-For* to simulate requests from *different IPs*. This technique is used to *bypass rate-limiting*.
