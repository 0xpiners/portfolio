...
Let's analyze it before SSH-ing into the machine.
![[Pasted image 20251115011518.png]]
It basically computes the MD5 hash of /root/flag.txt. Since I don't have any flag.txt on my machine, it gives an error.


![[Pasted image 20251115011449.png]]
After that, I used `strings` first to find any useful strings and found the command it uses to compute the MD5 hash.

It uses **md5sum**.

Let's SSH into the machine to find anything useful.
![[Pasted image 20251115011701.png]]
Found the location of the file.

I checked the permissions of the binary and for some reason it's writable.
![[Pasted image 20251115011819.png]]

Since it's writable, we can just:
