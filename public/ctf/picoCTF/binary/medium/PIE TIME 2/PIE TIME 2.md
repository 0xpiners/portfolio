...
Once again we have an ELF executable and the source code.
![[Pasted image 20251107175820.png]]
Source code of vuln.c:
![[Pasted image 20251107175942.png]]
As we can see in the main:
1. They prepare a signal for segmentation fault, which is not interesting since it's just a printf function.
2. Set the buffer for stdout as unbuffered, not important.
3. And then it calls the function `call_functions`.
...
For this, we first need to know the function offsets. Since this is an ELF PIE executable, the base address is randomized at runtime.
...
Now that we know some **offsets**, we can leak addresses until we find one ending with those values.

With the address and offset, we can calculate the **base address**, and from there calculate the **win address**.
With the win address, it's game over.
I made this script to make it easier to get the flag.
